...
public class ExamDbFacade_audio {
  ...
  //  public static OnDatabaseChangedListener mOnDatabaseChangedListener;
    ...
    /*public ArrayList<InfoClass_audio> insert(String title, String created_at,String path, String updated_at, String position, String document) {
        ...
        values.put(ExamDbContract_audio.ExamDbEntry., title);
...
/*    //입력한 id값을 가진 DB를 지우는 메소드
    ...
    public void editTitle(int id, String newTitle) {
        ...
        // 삽입할 문자열을 파라메터로 받아서 저장합니다.
        values.put(ExamDbContract_audio.ExamDbEntry.COLUMN_NAME_RECORDING_NAME, newTitle);
...
/*    public Cursor getTitle(long id){
    ...
    public Cursor getAll() {
        SQLiteDatabase db = mHelper.getReadableDatabase();
        Cursor c = db.rawQuery("SELECT _ID, recording_name, file_path, document, pos, created_at FROM " + ExamDbContract_audio.ExamDbEntry.TABLE_NAME+" order by _ID asc", null);
        return c;
    }
    ...
    public Cursor order_desc() {//최신순
        SQLiteDatabase db = mHelper.getReadableDatabase();
        Cursor c = db.rawQuery("SELECT _ID, recording_name, file_path, document, pos, created_at FROM " + ExamDbContract_audio.ExamDbEntry.TABLE_NAME +" order by _ID desc", null);
        return c;

    }
    public Cursor order_alp_asc() {//알파벳순
        SQLiteDatabase db = mHelper.getReadableDatabase();
        Cursor c = db.rawQuery("SELECT _ID, recording_name, file_path, document, pos, created_at FROM " + ExamDbContract_audio.ExamDbEntry.TABLE_NAME +" order by recording_name asc", null);
        return c;
    ...
    public Cursor order_doc_asc() {//문서별
        SQLiteDatabase db = mHelper.getReadableDatabase();
        Cursor c = db.rawQuery("SELECT _ID, recording_name, file_path, document, pos, created_at FROM " + ExamDbContract_audio.ExamDbEntry.TABLE_NAME +" order by document asc", null);
        return c;
    ...
    public int getCount() {
        SQLiteDatabase db = mHelper.getReadableDatabase();
        String[] projection = { ExamDbContract_audio.ExamDbEntry._ID };
        Cursor c = db.query(ExamDbContract_audio.ExamDbEntry.TABLE_NAME, projection, null, null, null, null, null);
        int count = c.getCount();
        c.close();
        return count;
    }

    public long addRecording(String recordingName, String filePath, long length, String document, String created_at, String pos) {

        SQLiteDatabase db = mHelper.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(ExamDbContract_audio.ExamDbEntry.COLUMN_NAME_RECORDING_NAME, recordingName);
        cv.put(ExamDbContract_audio.ExamDbEntry.COLUMN_NAME_RECORDING_FILE_PATH, filePath);
        cv.put(ExamDbContract_audio.ExamDbEntry.COLUMN_NAME_RECORDING_LENGTH, length);
        cv.put(ExamDbContract_audio.ExamDbEntry.COLUMN_NAME_TIME_ADDED, System.currentTimeMillis());
        cv.put(ExamDbContract_audio.ExamDbEntry.DOCUMENT, document);
        cv.put(ExamDbContract_audio.ExamDbEntry.CREATED_AT, created_at);
        cv.put(ExamDbContract_audio.ExamDbEntry.POS, pos);

        long rowId = db.insert(ExamDbContract_audio.ExamDbEntry.TABLE_NAME, null, cv);
